...
| `message_sent` | **经 OpenClaw 发出**的消息 → 同步到收件箱，保持记录完整 |
| `before_dispatch` | 返回 `{ handled: true }` 接管会话，阻止大模型自动回复 |

> `message_sent` 只在 OpenClaw 自己发消息时触发。你**在手机微信上直接回复**的内容不经过 OpenClaw，
> 这个钩子不会触发；这类消息会不会出现在收件箱，取决于腾讯 `getUpdates` 是否回传本账号自己发出的消息，
> 需要在真实环境确认。若确实回传，它会被当成入站消息（方向显示为对方发来），届时按实际字段调整判定即可。
